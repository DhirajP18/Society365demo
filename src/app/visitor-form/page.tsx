...
// ── Loading skeleton shown by Suspense while params resolve ───────────────────
function PageLoader() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50
      dark:from-[#07080f] dark:via-[#0a0c14] dark:to-[#080a12]
      ...
      <div className="bg-white dark:bg-[#0f1117] rounded-3xl border border-gray-200
        dark:border-white/[0.07] shadow-xl p-10 flex flex-col items-center gap-4 w-full max-w-md">
        <Loader2 className="h-10 w-10 text-blue-500 animate-spin" />
        <p className="text-[14px] font-semibold text-gray-500">Loading…</p>
      </div>
    </div>
  )
}
...
// wrapped in Suspense, otherwise the build crashes during static prerender.
// ═══════════════════════════════════════════════════════════════════════════════
function VisitorFormInner() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50
      dark:from-[#07080f] dark:via-[#0a0c14] dark:to-[#080a12]
      flex flex-col items-center justify-start px-4 py-8">
      ...
      <div className="w-full max-w-md">
        ...
        {state === "loading" && (
          <div className="bg-white dark:bg-[#0f1117] rounded-3xl border border-gray-200
            dark:border-white/[0.07] shadow-xl p-10 flex flex-col items-center gap-4">
            <Loader2 className="h-10 w-10 text-blue-500 animate-spin" />
        ...
        {state === "invalid" && (
          <div className="bg-white dark:bg-[#0f1117] rounded-3xl border border-gray-200
            dark:border-white/[0.07] shadow-xl p-8 flex flex-col items-center gap-4 text-center">
            <div className="h-16 w-16 rounded-2xl bg-rose-100 dark:bg-rose-500/10
              border border-rose-200 dark:border-rose-500/20 flex items-center justify-center">
        ...
        {/* ── Expired ── */}
        {state === "expired" && (
          <div className="bg-white dark:bg-[#0f1117] rounded-3xl border border-amber-200
            dark:border-amber-500/20 shadow-xl p-8 flex flex-col items-center gap-4 text-center">
            <div className="h-16 w-16 rounded-2xl bg-amber-100 dark:bg-amber-500/10
              border border-amber-200 dark:border-amber-500/20 flex items-center justify-center">
        ...
        {/* ── Already used ── */}
        {state === "used" && (
          <div className="bg-white dark:bg-[#0f1117] rounded-3xl border border-emerald-200
            dark:border-emerald-500/20 shadow-xl p-8 flex flex-col items-center gap-4 text-center">
            <div className="h-16 w-16 rounded-2xl bg-emerald-100 dark:bg-emerald-500/10
        ...
        {state === "success" && (
          <div className="bg-white dark:bg-[#0f1117] rounded-3xl border border-emerald-200
            dark:border-emerald-500/20 shadow-xl overflow-hidden animate-[fadeInUp_0.35s_ease_both]">
            <div className="h-1 bg-gradient-to-r from-emerald-400 to-teal-400" />
            <div className="p-8 flex flex-col items-center gap-4 text-center">
              <div className="flex h-20 w-20 items-center justify-center rounded-full
                bg-gradient-to-br from-emerald-400 to-teal-500
                shadow-[0_8px_24px_rgba(52,211,153,0.4)]
              ...
              <div className="w-full space-y-2 mt-2">
                {[
                  ...
                  { label: "Visiting",  value: selectedResident
                      ? `${selectedResident.name} (${selectedResident.flatNo})` : "",
                  ...
                  <div key={row.label}
                    ...
                    <span className="text-[13px] font-semibold text-gray-800 dark:text-white flex-1">
                      {row.value}
                    </span>
                  </div>
                ))}
        ...
        {state === "form" && (
          <div className="bg-white dark:bg-[#0f1117] rounded-3xl border border-gray-200
            dark:border-white/[0.07] shadow-xl overflow-hidden">
            ...
            <form onSubmit={submit} className="px-6 py-5 space-y-4">
              ...
              <FloatInput label="Mobile Number" value={mobile} onChange={setMobile}
                type="tel" icon={<Phone className="h-4 w-4" />} required />

              <FloatInput label="Designation / Purpose of Visit" value={designation}
                onChange={setDesig} icon={<Briefcase className="h-4 w-4" />} />

              <div className="space-y-1.5">
                <p className="text-[10.5px] font-bold uppercase tracking-wide text-gray-400
                  flex items-center gap-1.5">
                  <Home className="h-3.5 w-3.5" />Who are you visiting? *
                </p>
                <select value={visiting} onChange={e => setVisiting(+e.target.value)}
                  className={cn(
              ...
              <div className="rounded-xl border border-gray-100 dark:border-white/[0.07]
                bg-gray-50 dark:bg-white/[0.03] px-4 py-3.5 space-y-2.5">
                <p className="text-[10.5px] font-bold uppercase tracking-wide text-gray-400">
                  Notify resident via
                </p>
                <label className="flex items-center gap-3 cursor-pointer">
                  <div className={cn("relative h-5 w-9 rounded-full transition-all",
                    notifyEmail ? "bg-blue-500" : "bg-gray-300 dark:bg-gray-600")}
                    onClick={() => setNotifyEmail(v => !v)}>
                    <div className={cn("absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-all",
                      notifyEmail ? "left-[18px]" : "left-0.5")} />
                  </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <div className={cn("relative h-5 w-9 rounded-full transition-all",
                    notifySMS ? "bg-violet-500" : "bg-gray-300 dark:bg-gray-600")}
                    onClick={() => setNotifySMS(v => !v)}>
                    <div className={cn("absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-all",
                      notifySMS ? "left-[18px]" : "left-0.5")} />
                  </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// DEFAULT EXPORT — Suspense wrapper is REQUIRED for useSearchParams()
// Without this Next.js build crashes: "useSearchParams() should be wrapped
// in a suspense boundary at page /visitor-form"
// ═══════════════════════════════════════════════════════════════════════════════
